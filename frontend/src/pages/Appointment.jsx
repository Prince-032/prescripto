...
const Appointment = () => {
  ...
  const bookedAppointment = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(backendUrl + '/api/user/booked-appointment', { docId, slotDate, slotTime }, { headers: { token } });
  ...
  return docInfo && (
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          <img className='bg-indigo-500 w-full sm:max-w-72 rounded-lg' src={docInfo.image} alt="doc_image" />
        </div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <button onClick={bookedAppointment} className='bg-indigo-500 text-white text-sm font-light px-14 py-3 rounded-full my-6'>Book an appointment</button>
      </div>
